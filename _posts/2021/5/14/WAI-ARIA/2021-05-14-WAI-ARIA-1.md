...
단독으로 배포가능한, 재사용이 가능한 단위일 때 사용한다. 이러한 예는 신디케이션이 있다.
...
article은 위에서 언급한 바와 같이 **단독으로 배포가능한, 재사용이 가능한 단위일 때, 사용한다. **  반면, section은 의미론적으로 구획을 나누는 역할을 하는 것 이상의 역할을 하지 않는다.

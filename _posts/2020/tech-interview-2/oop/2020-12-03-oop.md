...
4. **다형성**  
이름은 같으나 시그니쳐가 다르오
> 하나의 메소드나 클래스가 여러 가지 형태로 사용될 수 있는 것  
**자바** 에서는 한 타입(Super Class / Interface) 의 참조변수로  
여러 타입의 객체 (Sub Class, 구현체) 를 참조할 수 있도록 함

- 오버로딩 / 오버라이딩  
- 한 타입의 참조변수로 여러 타입의 객체를 참조할 수 있도록 함 (인터페이스, 상속)

  **다형성을 쓰는 이유**  
  - 전략 패턴과 같이 유연한 프로그래밍이 가능하다.
  - 추상화를 지켜며, 유지보수의 측면에서 좋다

출처: https://arabiannight.tistory.com/entry/302 [아라비안나이트]
...
### 팩토리 메서드 패턴
> 객체의 생성 코드를 별도의 클래스/메서드로 분리하는 패턴

#### 팩토리 메서드 패턴의 개념과 적용 방법

1. 객체 생성을 전담하는 별도의 Factory 클래스 이용  
  스트래티지 패턴과 싱글턴 패턴을 이용한다.
    - 생성 Factory 클래스는 파라미터를 받아서 상황에 맞는 객체를 얻어온다.
2. 상속 이용:  
하위 클래스에서 적합한 클래스의 객체를 생성
스트래티지 패턴, 싱글턴 패턴과 템플릿 메서드 패턴을 이용한다.

## 3. 객체 지향의 5대 원칙 (SOLID)
> High Cohesion - Loose Coupling 원칙을 객체 지향애 도입한 것, 재사용은 극대화 시키되 수정은 최소화 하는것이 목표 ㄴ

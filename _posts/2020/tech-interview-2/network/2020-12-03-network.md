...
1. 흐름제어
    ...
    1. Stop And Wait
        ...
        > 매번 전송한 패킷에 대해 확인응답을 받아야 다음 패킷 전송

    2. Sliding Window

        > 수신측 응답에 따라 윈도우 (버퍼) 를 이용하여 흐름을 제어한다. 
...
7. OPTIONS
    - 요청한 URL이 어떤 http 메서드를 지원 하는지 체크할 때 사용한다 
    - response 헤더에 Allow 에 가능한 메서드가 적혀온다
    - 보안 위협
...
- 200 : 요청은 정상이고, 본문은 요청된 리소스를 포함하고 있다. (OK)

- 201 : 어떠한 생성 작업을 요청받았으며, 생성 작업을 성공하였다. (CREATED)
...
- 400 : 클라이언트가 올바르지 못한 요청을 보내고 있음을 의미 (BAD REQUEST)

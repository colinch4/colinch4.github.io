...
예를 들어, 숫자를 연산하는 제네릭 메소드는 매개값으루 Number 타입 또는 자식 클래스 타입(Byte, Short, Integer, Long, Double)의 인스턴스만 가져야 한다. 이것이 제한된 타입 파라미터가 필요한 이유이다.
...
-   제네릭타입<? extends 상위타입> :  **부모 클래스 제한**
    
-   제네릭타입<? super 하위타입> :  **자식 클래스 제한**